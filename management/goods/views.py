...
class GoodsView(View):
    ...
    def get(self,request,user):
        ...
        result = [{"id": goods.id, "name":goods.name_en,"description":goods.description_en,"price":goods.on_price,"category":goods.category.name,"super_category":goods.category.super_category.name,"sale":goods.actual_sale + goods.virtual_sale,"stock":goods.stock,"on_sale":goods.on_sale} for goods in goods_list]
    ...
    def post(self,request,user):
        ...
        try:
            goods = Goods.objects.create(name_en = name,detail_en=detail,description_en=description,on_sale=on_sale,origin_price=origin_price,on_price=on_price,
                                 added_time=added_time,stock=stock,virtual_sale=virtual_sale,is_hot=is_hot,is_new=is_new)
...
class GoodsDetailView(View):
    ...
    def get(self,request,user,goods_id):
        try:
            ...
            management_logger.error(e)
            return JsonResponse({"errcode": "102", "errmsg": "db error"})
        result = {"id":goods.id,"name":goods.name_en,"images":[settings.URL_PREFIX + image.image.url for image in images],
                  ...
                  "stock":goods.stock,"virtual_sale":goods.virtual_sale,"is_hot":goods.is_hot,"is_new":goods.is_new}
        return JsonResponse({"errcode":"0","data":result})
...
class GoodsSettingView(View):
    ...
    def put(self,request,user,goods_id):
        ...
        is_hot = data.get("is_hot",None)
        is_new = data.get("is_new",None)
        print(on_sale,added_time,origin_price,on_price,stock,virtual_sale)
        if on_sale is None or added_time is None or origin_price is None or on_price is None or stock is None\
            or virtual_sale is None or is_hot is None or is_new is None:
            return JsonResponse({"errcode": "101", "errmsg": "params error"})
        try:
            Goods.objects.filter(id=goods_id).update(on_sale=on_sale,added_time=added_time,origin_price=origin_price,on_price=on_price,
                                                             stock=stock,virtual_sale=virtual_sale,is_hot=is_hot,is_new=is_new)
        except Exception as e:
            management_logger.error(e)
            return JsonResponse({"errcode": "102", "errmsg": "db error"})
        return JsonResponse({"errcode": "0", "errmsg": "update success"})
...
class CategoryView(View):
    ...
    def put(self,request,user,category_id):
        ...
        try:
            ...
            management_logger.error(e)
            return JsonResponse({"errcode":"102","errmsg":"can noto find category in db"})
        except Exception as e:
            management_logger.error(e)
            return JsonResponse({"errcode":"102","errmsg":"db error"})
        return JsonResponse({"errcode":"0","errmsg":"update success"})
